	...
	public void render(IntBufferWrapper intBufferWrapper) {
		...
		if (exercise.ordinal() == ExerciseEnum.EX_3_3_Object_transformation___4_objects.ordinal()) {
			...
			if(projectionType==ProjectionTypeEnum.ORTHOGRAPHIC) {
				Matrix4f projectionMatrix = (new Matrix4f()).ortho(-1.5F, 1.5F, -1.5F, 1.5F, 0.0F, 100.0F);
				object1.setProjectionM(projectionMatrix);
			}
			...
			if(projectionType==ProjectionTypeEnum.PERSPECTIVE) {
			    ...
			    float zNear = 1.0F;
			    float zFar = 100.0F;
			    Matrix4f perspectiveMatrix = new Matrix4f().perspective((float) Math.toRadians(fovY), aspectRatio, zNear, zFar);
